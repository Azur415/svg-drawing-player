...
通过浏览器原生几何接口支持相对路径、圆弧、多子路径、基础形状、嵌套分组、变换、渐变和裁切。文字、内嵌 PNG/JPEG/GIF/WebP 与未标记的普通复杂效果淡入；普通的整体合成分组作为一个播放单元。带有 `data-trace-chapter`、`data-trace-order` 和 `data-trace-role="fill|stroke"` 标记的 trace-contract SVG，即使路径位于整体滤镜、仅透明度分组或局部滤镜内，也会继续逐路径播放；`data-trace-role="static"` 与 `data-trace-exclude="true"` 的元素不会进入时间轴。defs 内的定义不会被重复计为绘制对象。
