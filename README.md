...
Paths use the browser's SVG geometry APIs, including relative commands, arcs and multiple subpaths. Basic shapes, nested groups, transforms, gradients and clipping are supported. Text, local `use` instances, embedded PNG/JPEG/GIF/WebP and complex compositing effects fade in; ordinary affected groups remain atomic to preserve their compositing. Trace-contract SVGs may mark replayable leaves with `data-trace-chapter`, `data-trace-order` and `data-trace-role="fill|stroke"`; their marked paths remain individually replayable inside composition-filtered or opacity-only containers, while `data-trace-role="static"` and `data-trace-exclude="true"` stay out of the timeline. References in `defs` do not become duplicate drawing steps.
